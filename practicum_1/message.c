...
// Creates a new message with provided details
...
// Stores a message to disk as a text file
...
// Retrieves a message from disk by ID
...
// Adds message to cache based on specified replacement policy
...
// Stores message on disk and cache
...
// Retrieves a message, first checking cache then disk
Message* retrieve_msg(int id, const char* method) {
    ...
    if (cached_index != -1) {
        ...
        printf("Message %d reloaded from cache at index %d with access count %d\n", cached->id, cached_index, cache[cached_index].last_access);
        return cached;
    }
